...
function () {
  ...
  _createClass(Base, [{
    ...
    value: function setOptions() {
      ...
      Vue.set(this, '_options', defaultsDeep_1.apply(void 0, [{}].concat(options, [// Given options
...
function (_Base) {
  ...
  function Collection() {
    ...
    Vue.set(_assertThisInitialized(_this), 'models', []); // Model store.

    Vue.set(_assertThisInitialized(_this), '_attributes', {}); // Property store.

    Vue.set(_assertThisInitialized(_this), '_registry', {}); // Model registry.

    Vue.set(_assertThisInitialized(_this), '_page', NO_PAGE);
  ...
  _createClass(Collection, [{
    ...
    value: function clearModels() {
      ...
      Vue.set(this, 'models', []); // Notify each model that it has been removed from this collection.
    ...
    value: function sort(comparator) {
      Vue.set(this, 'models', sortBy_1(this.models, comparator));
    }
    /**
    ...
    value: function page(_page) {
      ...
      if (isNil_1(_page)) {
        Vue.set(this, '_page', NO_PAGE); // Page was provided, so we should either set the page or disable
        // pagination entirely if the page is `false`.
      } else {
        Vue.set(this, '_page', max_1([1, toSafeInteger_1(_page)]));
      }
    ...
    value: function applyPagination(models) {
      ...
      if (isEmpty_1(models)) {
        Vue.set(this, '_page', LAST_PAGE); // Otherwise, there were at least one model, and we can safely
...
var copyFrom = function copyFrom(source, target, keys) {
  ...
  each(source, function (value, key) {
    if (isArray_1(value)) {
      ...
      Vue.set(target, key, cloneDeep_1(value));
    }
  });
};
/**
...
function (_Base) {
  ...
  function Model() {
    ...
    Vue.set(_assertThisInitialized(_this), '_collections', {}); // Collections that contain this model.

    Vue.set(_assertThisInitialized(_this), '_reference', {}); // Saved attribute state.

    Vue.set(_assertThisInitialized(_this), '_attributes', {}); // Active attribute state.

    Vue.set(_assertThisInitialized(_this), '_mutations', {}); // Mutator cache.

    Vue.set(_assertThisInitialized(_this), '_errors', {}); // Validation errors.
  ...
  _createClass(Model, [{
    ...
    value: function clearAttributes() {
      ...
      Vue.set(this, '_attributes', cloneDeep_1(defaults));
      Vue.set(this, '_reference', cloneDeep_1(defaults));
    }
    /**
     * Reverts all attributes back to their defaults, and completely removes all
     * attributes that don't have defaults. This will also sync the reference
    ...
    value: function mutate(attribute) {
      ...
      if (isUndefined_1(attribute)) {
        each(this._attributes, function (value, attribute) {
          Vue.set(_this3._attributes, attribute, _this3.mutated(attribute, value));
        ...
        each(castArray_1(attribute), function (attribute) {
          ...
          Vue.set(_this3._attributes, attribute, mutated);
        });
      }
    }
    /**
    ...
    value: function sync(attribute) {
      ...
      if (isUndefined_1(attribute)) {
        ...
        each(castArray_1(attribute), function (attribute) {
          Vue.set(_this4._reference, attribute, get_1(active, attribute));
        });
      }
    ...
    value: function set(attribute, value) {
      ...
      Vue.set(this._attributes, attribute, value); // Only consider a change if the attribute was already defined.
    ...
    value: function unset(attribute) {
      ...
      each(castArray_1(attributes), function (attribute) {
        if (_this7.has(attribute)) {
          Vue.set(_this7._attributes, attribute, get_1(defaults, attribute));
        }
      });
    }
    /**
    ...
    value: function setAttributeErrors(attribute, errors) {
      if (isEmpty_1(errors)) {
        ...
        Vue.set(this._errors, attribute, castArray_1(errors));
      }
    }
    /**
    ...
    value: function onSaveSuccess(response) {
      ...
      Vue.set(this, 'saving', false);
      Vue.set(this, 'fatal', false); // Automatically add to all registered collections.
    ...
    value: function onSave() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        Vue.set(_this15, 'saving', true); // Mutate attribute before we save if required to do so.
  ...
  methods: {
    ...
    removeItem: function (id, index) {
      this.$bvModal.msgBoxConfirm(this.messageRemoveConfirm, {
        ...
        if (value) {
          this.loading = true;
          axios$4.delete(this.apiUrl + "/" + this.modelName + "/" + id).then(response => {
    ...
    async saveItem() {
      ...
      if (this.item.id) {
        axios$4.put(this.apiUrl + "/" + this.modelName + "/" + this.item.id, this.item).then(response => {
        ...
        if (this.createMultipart) {
          ...
          axios$4.post(this.apiUrl + "/" + this.modelName, formData).then(response => {
            this.loading = false;
            if (this.hideModalAfterSave) {
              this.$bvModal.hide("modal-form-item-" + this.modelName);
            }
          ...
          axios$4.post(this.apiUrl + "/" + this.modelName, this.item).then(response => {
            this.loading = false;
            if (this.hideModalAfterSave) {
              this.$bvModal.hide("modal-form-item-" + this.modelName);
            }
